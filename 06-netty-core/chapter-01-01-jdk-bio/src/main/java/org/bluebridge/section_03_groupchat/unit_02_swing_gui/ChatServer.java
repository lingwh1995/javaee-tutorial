package org.bluebridge.section_03_groupchat.unit_02_swing_gui;
...
class ChatServerThread extends Thread {
        ...
        try {
            ...
            // 2. 读取当前的消息类型：登录，群发，私聊，@消息
                ...
                if(flag == 1){
                    ...
                    ChatServer.onLineSockets.put(socket,name);
                }
                writeMsg(flag,dis);
            }
        }catch (Exception e){
    ...
    private void writeMsg(int flag, DataInputStream dis) throws Exception {
        ...
        if(flag == 1){
            ...
            Collection<String> onlineNames = ChatServer.onLineSockets.values();
            // 判断是否存在在线人数
            ...
            // 得到发件人
            String sendName = ChatServer.onLineSockets.get(socket);
            // 内容--
