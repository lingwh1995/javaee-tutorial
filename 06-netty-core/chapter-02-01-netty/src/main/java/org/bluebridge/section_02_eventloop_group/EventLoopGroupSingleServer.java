package org.bluebridge.section_02_eventloop_group;
 ...
 * 1. 默认线程数：NioEventLoopGroup 默认的线程数量是 CPU 核心数的两倍。
 * 2. 具体来说，其默认构造函数会使用 Runtime.getRuntime().availableProcessors() * 2 作为线程数。
 * 3. 这适用于大多数 I/O 密集型应用，能够较好地利用多核 CPU 性能。
 *
 * 构造方式
 * 1. new NioEventLoopGroup()：无参构造函数会自动设置线程数为 CPU 核心数的两倍。
 * 2. 也可以通过 new NioEventLoopGroup(int nThreads) 手动指定线程数。
 *
 * 适用场景
 * 1. 对于 I/O 密集型任务（如网络通信），默认值通常是合适的。
 * 2. 如果是计算密集型任务，可能需要根据实际情况调整线程数，避免过多线程导致上下文切换开销。
 *
 * 总结
